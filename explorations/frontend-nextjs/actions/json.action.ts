/**
 * Calls the backend API to generate JSON diagrams.
...
export async function generateJSONAction(
	description: string,
	diagramType: string,
	theme?: string,
	code?: string
	...
	try {
		...
		return {
			...
			image_url: null,
			status: 'error',
			message: error.message || 'Failed to generate JSON diagram',
		}
	}
}

export async function renderJSONAction(
	plantumlCode: string
): Promise<{
	image_base64: string | null,
	image_url: string | null,
	...
	try {
		...
		return {
			...
			image_url: null,
			status: 'error',
			message: error.message || 'Failed to render JSON diagram',
		}
	}
}
