/**
 * Calls the backend API to generate Gantt charts.
 * Returns: { plantuml_code: string, image_base64: string, image_url: string, status: string, message: string }
 */
const API_BASE_URL = '/api/gantt'

export async function generateGanttAction(
	description: string,
	diagramType: string,
	theme?: string,
	code?: string
	...
	try {
		...
		return {
			...
			image_url: null,
			status: 'error',
			message: error.message || 'Failed to generate Gantt chart',
		}
	}
}

export async function renderGanttAction(
	plantumlCode: string
): Promise<{
	image_base64: string | null,
	image_url: string | null,
	...
	try {
		...
		return {
			...
			image_url: null,
			status: 'error',
			message: error.message || 'Failed to render Gantt chart',
		}
	}
}
